...
class CreateSlot : AppCompatActivity() {
    ...
    private fun apiCall() {
        ...
        ApiClient.apiService.createSlot(sessionManager.id.toString(),startTime,endTime,selectedDate)
            .enqueue(object : Callback<ModelCreateSlot> {
                @SuppressLint("LogNotTimber")
                override fun onResponse(
                    call: Call<ModelCreateSlot>, response: Response<ModelCreateSlot>
                ) {
...
//                                finish()
                               // startActivity(intent)
                                ...
                                startActivity(Intent(this@CreateSlot,ManageSlots::class.java))
                            }
                            .setCancelClickListener { sDialog ->
                                sDialog.cancel()
                            }
                ...
                override fun onFailure(call: Call<ModelCreateSlot>, t: Throwable) {
                    myToast(this@CreateSlot,t.message.toString())
                    progressDialog!!.dismiss()
